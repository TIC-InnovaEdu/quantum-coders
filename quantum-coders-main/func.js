...
function playSceneMusic(sceneKey) {
    // No detener la música si ya es la música de menú y estamos mostrando el formulario
    const isShowingForm = !document.getElementById('score-form-container').classList.contains('hidden');
    if (currentAudio && (currentAudio.src.includes('Menu_m') && isShowingForm)) {
        return;
    }
...
function showGameOver() {
    ...
    // Guardar datos en localStorage para recuperarlos después del reload
    localStorage.setItem('showScoreForm', 'true');
    ...
    // Recargar después de un breve tiempo (volver al menú)
    setTimeout(() => {
        if (currentAudio) {
            currentAudio.pause();
            currentAudio.currentTime = 0;
...
function hideScoreForm() {
    const formContainer = document.getElementById('score-form-container');
    if (formContainer) {
...
async function handleScoreSubmit(event) {
  ...
  if (!playerName) {
    alert('Por favor ingresa tu nombre o apellido');
    return;
  }
  ...
  try {
    ...
    const q = window.query(
      ...
      window.orderBy("numero", "desc"),
      window.limit(1)
    );
    const lastSnap = await window.getDocs(q);

    let nextNumero = 1;
    if (!lastSnap.empty) {
      const lastNumero = lastSnap.docs[0].data().numero || 0;
    ...
    await window.addDoc(window.collection(window.db, "scores"), {
      ...
      completado: gameCompleted
    });

    alert(`¡Puntuación de ${finalScore} guardada para ${playerName}!`);
    hideScoreForm();
    finalScore = 0;
    gameCompleted = false;
    ...
    alert("Hubo un problema guardando tu puntuación.");
  }
}

// Función para calcular la puntuación final
function calculateFinalScore() {
    finalScore = wisdomPoints * 10 + playerLives * 20;
    
    // Bonus por completar el juego
    if (scene === 8 && gameCompleted) {
        finalScore += 100;
    }
    
    console.log('Puntuación calculada:', finalScore);
    return finalScore;
}

// Función para mostrar el formulario de puntuación
// Función para mostrar el formulario de puntuación
function showScoreForm() {
    console.log('Ejecutando showScoreForm');
    
    // Asegurarse de que estamos en el menú principal
    document.getElementById("menu").style.display = "flex";
    
    // Detener cualquier música de juego y reproducir música de menú
    if (currentAudio && !currentAudio.src.includes('Menu_m')) {
        currentAudio.pause();
        currentAudio.currentTime = 0;
        playSceneMusic('menu');
    }
    
    // Actualizar el puntaje en el formulario
    document.getElementById('final-score').textContent = finalScore;
    
    // Mensaje personalizado según si completó el juego o no
    const formTitle = document.querySelector('#score-form h2');
    if (gameCompleted) {
        formTitle.textContent = "¡Felicidades! Completaste el juego";
    } else {
        formTitle.textContent = "¡Juego Terminado!";
    }
...
// Función para cuando se completa el juego
function showFinalThanksAndReturnToMenu() {
    ...
    showRunaNarration(finalText, () => {
        ...
        console.log('Juego completado - Guardando puntuación:', finalScore);
        
        // Recargar la página (volver al menú)
        setTimeout(() => {
            if (currentAudio) {
                currentAudio.pause();
                currentAudio.currentTime = 0;
...
// Verificar si debemos mostrar el formulario al cargar la página
function checkShowScoreForm() {
    const shouldShowForm = localStorage.getItem('showScoreForm');
    const savedScore = localStorage.getItem('finalScore');
    const wasGameCompleted = localStorage.getItem('gameCompleted');
    
    console.log('Verificando formulario al cargar:', {shouldShowForm, savedScore, wasGameCompleted});
    
    if (shouldShowForm === 'true' && savedScore) {
        // Limpiar el localStorage inmediatamente
        localStorage.removeItem('showScoreForm');
        localStorage.removeItem('finalScore');
        localStorage.removeItem('gameCompleted');
        
        // Configurar variables globales
        finalScore = parseInt(savedScore);
        gameCompleted = (wasGameCompleted === 'true');
        
        console.log('Mostrando formulario con puntuación:', finalScore);
        
        // Pequeña espera para asegurar que el menú esté completamente cargado
        setTimeout(() => {
            showScoreForm();
        }, 300);
    }
...
// Agrega los event listeners para el formulario
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (cancelButton) {
        cancelButton.addEventListener('click', hideScoreForm);
    } else {
